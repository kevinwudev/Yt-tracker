...
def parse():
    ...
    parser.add_argument(
        "-d", "--datetime", help="The dateTime for video published, YYYY-MM-DD"
    )
    parser.add_argument(
        "-t", "--telegram", help="Report to telegram?", action="store_true"
    )
...
def main(date: str):
    ...
    for video in videos:
        ...
        video["summary"] = (
            summarize(transcript, lines=5).choices[0].message.content
            if transcript != ""
            else ""
        )

        res = [
            f" 創作者 : {video.get('videoOwnerChannelTitle', '')}",
